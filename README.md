...
- 设计是后台进程来定时探测，所以如果你的 Claude Code 指示挂着且没有交互，就不会在设计的窗口期间，刷新网络数据。
- 网络状况好坏是动态的（🟢/🟡/🔴），因此使用了 P95 统计值，在归集的 12 个 Total（端到端）数据基础上计算，更多细节自行查阅 `ccstatus-monitoring.json` 文件
- 如果降级/出错，会显示详细数据（DNS|TCP|TLC|ServerTTFB/TotaTTFB｜Total），特别是 TTFB，是从 `发出模型服务请求 <--> Anthropic 返回` 第一个字节的时间。
- 这中间是那个关卡的问题，这个工具仅能告诉你信息，你要自己去排查那个环节延迟最大。 

### OAuth 环境支持 ✨ (v2.2.6)
- **智能模式检测**: 自动识别 OAuth 环境并优化显示
  - **隐藏状态灯**: 隐藏网络状态 emoji (🟢/🟡/🔴/⚪) 和代理健康前缀
  - **跳过代理检查**: 自动跳过代理健康检查，避免不必要的网络请求
  - **保留时间指标**: 继续显示 P95 延迟、时间分解和 HTTP 版本等性能数据
  - **自动适配**: 通过 `api_config.source == "oauth"` 自动切换渲染模式
