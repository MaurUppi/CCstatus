    ...
    if let Ok(cm) = CredentialManager::new() {
        ...
        match result {
            Ok(Some(creds)) => {
                ...
                println!("Environment credentials test passed - found credentials from: {:?}", creds.source);
            },
            Ok(None) => {
                println!("No credentials found - this can happen in isolated test environments");
            },
            Err(e) => {
                println!("Error occurred (may be expected in some test environments): {}", e);
            }
        }
    }
...
"#;
    ...
    let cm = CredentialManager::new();
    assert!(cm.is_ok(), "CredentialManager should be created successfully");
...
"#;
    ...
    let result = cm.get_credentials().await;
    assert!(result.is_ok(), "Alternative config format should be handled");
...
"#;
    ...
    let result = cm.get_credentials().await;
    assert!(result.is_ok(), "Credential manager should handle malformed configs gracefully");
...
fn test_shell_detection() {
    ...
    assert!(matches!(detected, ShellType::Bash | ShellType::Zsh | ShellType::PowerShell | ShellType::Unknown));
}
...
fn test_shell_config_paths() {
    ...
    if zsh_paths.is_ok() {
        ...
        assert!(!paths.is_empty()); 
        assert!(paths.iter().any(|p| p.to_string_lossy().contains(".zshrc")
...
"#;
    ...
    let result = cm.get_credentials().await;
    assert!(result.is_ok(), "Credential manager should handle various quote styles and comments");
        ...
        match result {
            Ok(Some(creds)) => {
                ...
                if creds.base_url == "https://env-priority.anthropic.com" {
                    ...
                    println!("Priority hierarchy test passed - environment variables took precedence");
                } else {
        ...
        Ok(cm) => {
            ...
            match result {
                Ok(Some(creds)) => {
                    ...
                    assert!(!creds.auth_token.is_empty(), "Auth token should not be empty");
                    assert!(creds.base_url.starts_with("http"), "Base URL should be a valid URL");
                    
                    // Validate source is one of expected types
                    match creds.source {
                        CredentialSource::Environment => println!("Found environment credentials"),
                        CredentialSource::ShellConfig(ref path) => println!("Found shell credentials at: {:?}", path),
                        CredentialSource::ClaudeConfig(ref path) => println!("Found Claude config credentials at: {:?}", path),
                    }
...
fn test_helper_functions() {
    use ccstatus::core::segments::network::credential::{process_anthropic_variable, process_powershell_regex_captures};
    use regex::Regex;
    ...
    process_anthropic_variable(Some("ANTHROPIC_BASE_URL"), "https://helper-test.anthropic.com".to_string(), &mut base_url, &mut auth_token);
    process_anthropic_variable(Some("ANTHROPIC_AUTH_TOKEN"), "sk-helper-token".to_string(), &mut base_url, &mut auth_token);
    process_anthropic_variable(Some("OTHER_VAR"), "other_value".to_string(), &mut base_url, &mut auth_token);
    
    assert_eq!(base_url, Some("https://helper-test.anthropic.com".to_string()));
    ...
    process_powershell_regex_captures(&regex, r#"$env:ANTHROPIC_BASE_URL = "https://ps-helper-test.anthropic.com""#, &mut ps_base_url, &mut ps_auth_token);
    process_powershell_regex_captures(&regex, r#"$env:ANTHROPIC_AUTH_TOKEN = "sk-ps-helper-token""#, &mut ps_base_url, &mut ps_auth_token);
    
    assert_eq!(ps_base_url, Some("https://ps-helper-test.anthropic.com".to_string()));
