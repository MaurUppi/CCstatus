...
use ccstatus::core::segments::network::{
    NetworkSegment,
    types::{NetworkStatus, MonitoringState},
};
use ccstatus::core::segments::{Segment, SegmentData};
use ccstatus::config::{InputData, Model, Workspace, CostData};
...
mod window_boundary_tests {
    ...
    fn test_green_window_boundaries() {
        ...
        for (duration_ms, expected_in_window, description) in test_cases {
            ...
            assert!(result.is_some(), "Failed for {}: duration={}ms", description, duration_ms);
            
            println!("✓ {}: duration={}ms, expected_in_window={}", description, duration_ms, expected_in_window);
        }
    }
    ...
    fn test_cold_window_boundaries() {
        ...
        let test_cases = vec![
            (0, Some("uuid1".to_string()), true, "Start of COLD window with UUID"),
            (2500, Some("uuid2".to_string()), true, "Middle of COLD window with UUID"),
            (4999, Some("uuid3".to_string()), true, "End of COLD window with UUID"),
            (5000, Some("uuid4".to_string()), false, "Just after COLD window with UUID"),
            (5001, Some("uuid5".to_string()), false, "Well after COLD window with UUID"),
        ...
        for (duration_ms, parent_uuid, expected_cold_eligible, description) in test_cases {
            ...
            println!("✓ {}: duration={}ms, has_uuid={}, expected_cold_eligible={}", 
                description, duration_ms, input.parent_uuid.is_some(), expected_cold_eligible);
        }
    }
    ...
    fn test_window_id_calculation() {
        ...
        for (duration_ms, expected_window_id, description) in green_test_cases {
            ...
            assert_eq!(calculated_window_id, expected_window_id, 
                "GREEN window ID calculation failed for {}", description);
            
            println!("✓ {}: duration={}ms, window_id={}", description, duration_ms, calculated_window_id);
        }
        ...
        for (duration_ms, expected_window_id, description) in red_test_cases {
            ...
            assert_eq!(calculated_window_id, expected_window_id,
                "RED window ID calculation failed for {}", description);
    ...
    fn test_cold_window_env_config() {
        ...
        let input_within_custom = create_test_input_data(2999, Some("custom_uuid_1".to_string()), None);
        let input_outside_custom = create_test_input_data(3001, Some("custom_uuid_2".to_string()), None);
        ...
        assert!(result2.is_some(), "Custom COLD window (outside) test failed");
...
mod integration_tests {
    ...
    fn test_backward_compatibility() {
        ...
        let input = InputData {
            model: Model { display_name: "test-model".to_string() },
    ...
    fn test_no_credentials_behavior() {
        ...
        assert!(result.is_some(), "No credentials test should still return a result");
        
        let segment_data = result.unwrap();
        assert_eq!(segment_data.secondary, "network");
        
        // Check that has_credentials metadata reflects the lack of credentials
        let has_credentials = segment_data.metadata.get("has_credentials")
            .map(|v| v == "true")
            .unwrap_or(false);
        
        println!("✓ No credentials behavior test passed, has_credentials={}", has_credentials);
    }
    ...
    fn test_rapid_successive_calls() {
        ...
        for duration_ms in test_intervals {
            ...
            let result = segment.collect(&input);
            assert!(result.is_some(), "Rapid call test failed at duration={}ms", duration_ms);
    ...
    fn test_segment_data_structure() {
        ...
        assert!(!segment_data.primary.is_empty(), "Primary field should not be empty");
        assert_eq!(segment_data.secondary, "network", "Secondary field should be 'network'");
        
        // Verify required metadata fields
        let required_metadata = vec!["status", "p95_ms", "samples", "has_credentials"];
        for key in required_metadata {
            assert!(segment_data.metadata.contains_key(key), 
                "Required metadata key '{}' is missing", key);
        }
        
        println!("✓ Segment data structure test passed");
        println!("  Primary: '{}'", segment_data.primary);
        println!("  Metadata keys: {:?}", segment_data.metadata.keys().collect::<Vec<_>>());
    }
}