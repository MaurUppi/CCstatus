...
async fn test_debug_logging_disabled() {
    ...
    logger.performance("test_component", "test_operation", 100).await;
    logger.credential_info("test_component", "Environment", 20).await;
}
...
async fn test_debug_logging_enabled() {
    ...
    // Log some messages
    logger.debug("NetworkSegment", "Starting network monitoring").await;
...
async fn test_credential_info_logging() {
    ...
    logger.credential_info("CredentialManager", "Environment", 45).await;
...
fn test_debug_logger_default_trait() {
    ...
    assert!(logger.is_enabled() == env::var("CCSTATUS_DEBUG").map(|v| v == "true").unwrap_or(false));
}