use ccstatus::updater::url_resolver::{resolve_manifest_url, extract_host_from_url, try_urls_in_sequence, UrlResolverError};
...
fn test_master_branch_alignment() {
    ...
    for url in china_urls.iter().chain(non_china_urls.iter()) {
        if url.contains("raw.githubusercontent.com") {
            assert!(url.contains("/master/"), "URL should reference master branch: {}", url);
            assert!(!url.contains("/main/"), "URL should not reference main branch: {}", url);
        }
        if url.contains("jsdelivr.net") {
            assert!(url.contains("@master"), "CDN URL should reference @master: {}", url);
        }
    }
}
...
fn test_hk_proxy_url_format() {
    ...
    assert_eq!(github_url, "https://raw.githubusercontent.com/MaurUppi/CCstatus/master/latest.json");
}
...
fn test_jsdelivr_cdn_fallback() {
    ...
    assert!(non_china_urls.iter().any(|url| url.contains("jsdelivr.net")));
