...
fn test_version_prompt_deduplication() {
    ...
    state
        ...
        .insert("2.2.5".to_string(), yesterday);
...
fn test_etag_and_last_modified_caching() {
    ...
    state.set_last_modified(
        "raw.githubusercontent.com".to_string(),
        "Wed, 21 Oct 2015 07:28:00 GMT".to_string(),
    );
    state.set_last_modified(
        "hk.gh-proxy.com".to_string(),
        "Thu, 22 Oct 2015 08:30:00 GMT".to_string(),
    );

    // Verify host isolation
    assert_eq!(
        state.get_etag("raw.githubusercontent.com"),
        Some(&"W/\"abc123\"".to_string())
    );
    assert_eq!(
        state.get_etag("hk.gh-proxy.com"),
        Some(&"W/\"def456\"".to_string())
    );
    assert_eq!(state.get_etag("unknown.com"), None);

    assert_eq!(
        state.get_last_modified("raw.githubusercontent.com"),
        Some(&"Wed, 21 Oct 2015 07:28:00 GMT".to_string())
    );
    assert_eq!(
        state.get_last_modified("hk.gh-proxy.com"),
