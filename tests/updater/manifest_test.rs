...
fn test_persistent_cache_return_values_304_not_modified() {
    ...
    assert!(
        manifest_opt.is_none(),
        "304 response should return None for manifest"
    );
    assert!(
        etag_opt.is_none(),
        "304 response should return None for etag"
    );
    assert!(
        last_modified_opt.is_none(),
        "304 response should return None for last_modified"
    );
...
fn test_cache_persistence_state_update() {
    ...
    state
        .last_modified_map
        .insert(host.clone(), new_last_modified.clone());

    // Verify cache was updated
    assert_eq!(
        state.etag_map.get("raw.githubusercontent.com"),
        Some(&new_etag)
    );
    assert_eq!(
        state.last_modified_map.get("raw.githubusercontent.com"),
...
fn test_cache_persistence_round_trip() {
    ...
    state
        .last_modified_map
        .insert(host.clone(), last_modified.clone());
