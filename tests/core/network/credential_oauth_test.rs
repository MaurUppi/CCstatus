...
async fn test_oauth_credentials_when_present() {
    ...
    let result = cm.get_credentials().await.expect("Credential lookup should not fail");

    if let Some(creds) = result {
        // When OAuth is present, should return fixed credentials
        assert_eq!(creds.base_url, "https://api.anthropic.com", "OAuth should use official Anthropic API endpoint");
        assert_eq!(creds.auth_token, "probe-invalid-key", "OAuth should use dummy probe key");
        assert!(matches!(creds.source, CredentialSource::OAuth), "Source should be OAuth");
    } else {
...
async fn test_oauth_priority_over_shell_and_config() {
    ...
    if let Some(creds) = result {
        assert!(matches!(creds.source, CredentialSource::OAuth), 
            "OAuth should have priority over shell and config sources");
...
async fn test_oauth_fallback_when_not_present() {
    ...
    match result {
        ...
        Some(creds) => {
            ...
            println!("Found credentials from non-OAuth source: {:?}", creds.source);
        }
    }
}
...
async fn test_env_overrides_oauth() {
    ...
    assert_eq!(creds.base_url, "https://env-override.com", "Environment should override OAuth");
    assert_eq!(creds.auth_token, "env-override-token", "Environment should override OAuth");
    assert!(matches!(creds.source, CredentialSource::Environment), "Source should be Environment, not OAuth");
...
async fn test_oauth_skipped_on_non_macos() {
    ...
    match result {
        ...
        Some(creds) => {
            ...
            println!("Found credentials from non-OAuth source on non-macOS: {:?}", creds.source);
        }
    }
...
async fn test_oauth_error_handling() {
    ...
    assert!(result.is_ok(), "Credential lookup should not fail even if OAuth has errors");
