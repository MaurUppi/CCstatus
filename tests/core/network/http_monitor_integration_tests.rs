...
use ccstatus::core::network::http_monitor::{
    HttpMonitor, HealthCheckClient, HealthResponse
};
use ccstatus::core::network::status_renderer::StatusRenderer;
use ccstatus::core::network::types::{NetworkStatus, NetworkMetrics};
...
impl IntegrationMockHealthClient {
    ...
    fn set_error(&mut self, url: &str) {
        self.responses.insert(url.to_string(), Err("Network error".to_string()));
    }
...
impl HealthCheckClient for IntegrationMockHealthClient {
    async fn get_health(
        ...
        self.captured_methods.lock().unwrap().push("GET".to_string());
...
async fn integration_test_get_method_usage_validation() {
    ...
    let result = monitor.check_proxy_health_with_client("https://proxy.example.com", &mock_client).await;
    ...
    assert_eq!(captured_methods[0], "GET", "Health check must use GET method, not POST");
}
...
async fn integration_test_json_validation_case_insensitive() {
    ...
    let test_cases = vec![
        // Standard cases
        (br#"{"status": "healthy"}"#.to_vec(), true, "Standard healthy JSON"),
        (br#"{"status": "unhealthy"}"#.to_vec(), false, "Standard unhealthy JSON"),
        
        // Case insensitive value matching  
        (br#"{"status": "HEALTHY"}"#.to_vec(), true, "Uppercase healthy value"),
        (br#"{"status": "Healthy"}"#.to_vec(), true, "Title case healthy value"),
        (br#"{"status": "HeAlThY"}"#.to_vec(), true, "Mixed case healthy value"),
        
        // Case insensitive field name matching
        (br#"{"STATUS": "healthy"}"#.to_vec(), true, "Uppercase field name"),
        (br#"{"Status": "healthy"}"#.to_vec(), true, "Title case field name"),
        (br#"{"StAtUs": "healthy"}"#.to_vec(), true, "Mixed case field name"),
        
        // Extra fields tolerated
        (br#"{"status": "healthy", "version": "1.0"}"#.to_vec(), true, "Extra fields present"),
        (br#"{"timestamp": "2023-01-01", "status": "healthy", "uptime": 3600}"#.to_vec(), true, "Multiple extra fields"),
        
        // Invalid JSON cases
        (b"not json".to_vec(), false, "Malformed JSON"),
        (br#"{"status": "unknown"}"#.to_vec(), false, "Unknown status value"),
    ...
    for (body, expected_healthy, description) in test_cases {
        ...
        assert_eq!(
            ...
            if expected_healthy { "healthy" } else { "unhealthy" }
        );
    }
}
...
async fn integration_test_status_code_mapping_complete() {
    ...
    let status_code_mappings = vec![
        // Success codes
        (200, Some(true), "Only 200 with valid JSON should be healthy"),
        ...
        (301, Some(false), "301 Moved Permanently should be unhealthy"),
        (302, Some(false), "302 Found should be unhealthy"),
        (307, Some(false), "307 Temporary Redirect should be unhealthy"),
        (308, Some(false), "308 Permanent Redirect should be unhealthy"),
        ...
        // Server error codes
        (500, Some(false), "500 Internal Server Error should be unhealthy"),
        (502, Some(false), "502 Bad Gateway should be unhealthy"),
        (503, Some(false), "503 Service Unavailable should be unhealthy"),
...
async fn integration_test_end_to_end_status_rendering() {
    ...
    for (proxy_healthy, network_status, _expected_prefix, description) in test_scenarios {
        let metrics = NetworkMetrics {
            ...
            last_http_status: if proxy_healthy == Some(true) { 200 } else { 500 },
        ...
        if proxy_healthy == Some(true) {
            assert!(rendered.starts_with("🟢 |"), "{}: {}", description, rendered);
...
async fn integration_test_network_error_handling() {
    ...
    assert_eq!(result.unwrap(), Some(false), "Network errors should be unhealthy");
    
    // Validate GET method was attempted before error
    let captured_methods = mock_client.get_captured_methods();
    assert_eq!(captured_methods.len(), 1);
    assert_eq!(captured_methods[0], "GET", "Should attempt GET before failing");
}
...
async fn integration_test_all_success_criteria_met() {
    ...
    let captured_methods = mock_client.get_captured_methods();
    assert_eq!(captured_methods[0], "GET", "✅ Success Criteria 1: GET method used");
    ...
    assert_eq!(health_result, Some(true), "✅ Success Criteria 2: JSON validation works");
    
    // 3. Status code mapping correct (200 with valid JSON = healthy) ✅
    assert_eq!(health_result, Some(true), "✅ Success Criteria 3: 200 + valid JSON = healthy");
...
async fn integration_test_full_probe_to_render_pipeline() {
    ...
    mock_health_client.set_response(
        "https://my-proxy.example.com/health",
        200,
        120,
        health_body,
    );
    ...
    assert_eq!(captured_methods.len(), 1, "Health check should be called once");
    assert_eq!(captured_methods[0], "GET", "✅ GET method used in full pipeline");
    
    // Validate main probe succeeded
    assert_eq!(outcome.status, ccstatus::core::network::types::NetworkStatus::Healthy);
    ...
    let renderer = StatusRenderer::new();
    let rendered = renderer.render_status(
...
async fn integration_test_full_probe_unhealthy_proxy() {
    ...
    assert!(result.is_ok(), "Main probe can succeed even with unhealthy proxy");
    
    // Validate GET method was used
    let captured_methods = mock_health_client.get_captured_methods();
    assert_eq!(captured_methods[0], "GET", "GET method used for unhealthy proxy check");
...
impl TestMockHttpClient {
    ...
    fn set_response(&mut self, url: &str, status: u16, latency_ms: u64, breakdown: &str) {
        self.responses.insert(
            url.to_string(),
            (status, latency_ms, breakdown.to_string()),
        );
    }
}
