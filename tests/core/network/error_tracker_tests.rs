use ccstatus::core::segments::network::{
    types::{JsonlError, NetworkStatus},
    ErrorTracker,
};
...
fn test_max_history_limit() {
    ...
    assert!(
        ...
        "Error history should not exceed max limit"
    );
}
...
fn test_connection_error_classification_production_patterns() {
    ...
    assert_eq!(
        ErrorTracker::classify_connection_error("API Error (Connection error.)"),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("TypeError (fetch failed)"),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("network error occurred"),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("timeout during request"),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("connection refused by server"),
        "connection_error"
    );

    // New timeout patterns from CC-ErrorCode-1.png
    assert_eq!(
        ErrorTracker::classify_connection_error("Request timed out."),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("API Error (Request timed out.)"),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("Retrying in 1 seconds... (attempt 1/10)"),
        "connection_error"
    );

    // New SSL/Certificate patterns from CC-ErrorCode-1.png
    assert_eq!(
        ErrorTracker::classify_connection_error("unknown certificate verification error"),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("Error (unknown certificate verification error)"),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("TLS handshake failed"),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("SSL connection error"),
        "connection_error"
    );
    ...
    assert_eq!(
        ErrorTracker::classify_connection_error("usage policy violation detected"),
        "invalid_request_error"
    );

    // Test fallback - was "network_error", now "unknown_error" for spec compliance
    assert_eq!(
        ErrorTracker::classify_connection_error("some random error message"),
        "unknown_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("unexpected system failure"),
        "unknown_error"
    );

    // Test case insensitivity
    assert_eq!(
        ErrorTracker::classify_connection_error("CONNECTION ERROR"),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("FETCH FAILED"),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("REQUEST TIMED OUT"),
    ...
    assert_eq!(
        ErrorTracker::classify_connection_error("CERTIFICATE VERIFICATION"),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("USAGE POLICY"),
        "invalid_request_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("SSL ERROR"),
        "connection_error"
    );
    assert_eq!(
        ErrorTracker::classify_connection_error("TLS HANDSHAKE"),
        "connection_error"
    );
}
