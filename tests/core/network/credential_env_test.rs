...
async fn test_env_primary_combination() {
    ...
    let result = cm.get_from_environment().expect("Environment check should not fail");

    assert!(result.is_some(), "Should find primary environment combination");
...
async fn test_env_secondary_combination() {
    ...
    env::set_var("ANTHROPIC_BEDROCK_BASE_URL", "https://bedrock.amazonaws.com");
    env::set_var("ANTHROPIC_API_KEY", "test-api-key-456");

    let cm = CredentialManager::new().expect("Failed to create CredentialManager");
    let result = cm.get_from_environment().expect("Environment check should not fail");

    assert!(result.is_some(), "Should find secondary environment combination");
    let creds = result.unwrap();
...
async fn test_env_tertiary_combination() {
    ...
    env::set_var("ANTHROPIC_VERTEX_BASE_URL", "https://vertex-ai.googleapis.com");
    env::set_var("ANTHROPIC_API_KEY", "test-vertex-key-789");

    let cm = CredentialManager::new().expect("Failed to create CredentialManager");
    let result = cm.get_from_environment().expect("Environment check should not fail");

    assert!(result.is_some(), "Should find tertiary environment combination");
    let creds = result.unwrap();
...
async fn test_env_incomplete_combinations() {
    ...
    let cm = CredentialManager::new().expect("Failed to create CredentialManager");
    let result = cm.get_from_environment().expect("Environment check should not fail");
    assert!(result.is_none(), "Should not find credentials with only base URL");
    ...
    let result = cm.get_from_environment().expect("Environment check should not fail");
    assert!(result.is_none(), "Should not find credentials with only token");
}
...
async fn test_env_priority_rules() {
    ...
    env::set_var("ANTHROPIC_BEDROCK_BASE_URL", "https://bedrock.amazonaws.com"); // Secondary
    env::set_var("ANTHROPIC_VERTEX_BASE_URL", "https://vertex-ai.googleapis.com"); // Tertiary
    ...
    assert!(result.is_some(), "Should find credentials when all are present");
    let creds = result.unwrap();
    
    // Should prefer primary base URL and primary token
    assert_eq!(creds.base_url, "https://api.anthropic.com", "Should prefer ANTHROPIC_BASE_URL");
    assert_eq!(creds.auth_token, "primary-token-123", "Should prefer ANTHROPIC_AUTH_TOKEN");
    assert!(matches!(creds.source, CredentialSource::Environment));
}
...
async fn test_env_mixed_priority_combinations() {
    ...
    env::set_var("ANTHROPIC_BEDROCK_BASE_URL", "https://bedrock.amazonaws.com"); // Secondary base URL
    ...
    let creds = result.unwrap();
    assert_eq!(creds.base_url, "https://bedrock.amazonaws.com", "Should use available secondary base URL");
    assert_eq!(creds.auth_token, "primary-token-123", "Should use primary token");
    assert!(matches!(creds.source, CredentialSource::Environment));
}
...
async fn test_env_all_secondary_combinations() {
    ...
    env::set_var("ANTHROPIC_VERTEX_BASE_URL", "https://vertex-ai.googleapis.com"); // Tertiary base URL
    ...
    let creds = result.unwrap();
    assert_eq!(creds.base_url, "https://vertex-ai.googleapis.com", "Should use tertiary base URL");
    assert_eq!(creds.auth_token, "secondary-api-key-456", "Should use secondary token");
    assert!(matches!(creds.source, CredentialSource::Environment));
}
...
async fn test_env_empty_values() {
    ...
    assert!(result.is_none(), "Should not find credentials with empty base URL");
    ...
    let result = cm.get_from_environment().expect("Environment check should not fail");
    assert!(result.is_none(), "Should not find credentials with empty token");
}