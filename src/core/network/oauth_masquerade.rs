...
fn build_headers(opts: &OauthMasqueradeOptions) -> HashMap<String, String> {
    ...
    headers.insert("Authorization".to_string(), format!("Bearer {}", opts.access_token));
    
    // Core headers
    headers.insert("Content-Type".to_string(), DEFAULT_HEADER_PROFILE.content_type.to_string());
    headers.insert("Accept".to_string(), DEFAULT_HEADER_PROFILE.accept.to_string());
    headers.insert("anthropic-version".to_string(), DEFAULT_HEADER_PROFILE.anthropic_version.to_string());
    ...
    headers.insert("anthropic-dangerous-direct-browser-access".to_string(), 
                   DEFAULT_HEADER_PROFILE.dangerous_direct_browser_access.to_string());
    headers.insert("x-app".to_string(), DEFAULT_HEADER_PROFILE.x_app.to_string());
    
    // X-Stainless headers (test overrides available)
    headers.insert("X-Stainless-Retry-Count".to_string(), DEFAULT_HEADER_PROFILE.x_stainless_retry_count.to_string());
    headers.insert("X-Stainless-Timeout".to_string(), DEFAULT_HEADER_PROFILE.x_stainless_timeout.to_string());
    headers.insert("X-Stainless-Lang".to_string(), DEFAULT_HEADER_PROFILE.x_stainless_lang.to_string());
    headers.insert("X-Stainless-Package-Version".to_string(), DEFAULT_HEADER_PROFILE.x_stainless_package_version.to_string());
    
    let os = env::var(TEST_STAINLESS_OS).unwrap_or_else(|_| DEFAULT_HEADER_PROFILE.x_stainless_os.to_string());
    headers.insert("X-Stainless-OS".to_string(), os);
    
    let arch = env::var(TEST_STAINLESS_ARCH).unwrap_or_else(|_| DEFAULT_HEADER_PROFILE.x_stainless_arch.to_string());
    headers.insert("X-Stainless-Arch".to_string(), arch);
    
    let runtime = env::var(TEST_STAINLESS_RUNTIME).unwrap_or_else(|_| DEFAULT_HEADER_PROFILE.x_stainless_runtime.to_string());
    headers.insert("X-Stainless-Runtime".to_string(), runtime);
    
    let runtime_version = env::var(TEST_STAINLESS_RUNTIME_VERSION).unwrap_or_else(|_| DEFAULT_HEADER_PROFILE.x_stainless_runtime_version.to_string());
    headers.insert("X-Stainless-Runtime-Version".to_string(), runtime_version);
    
    // Accept headers
    headers.insert("accept-language".to_string(), DEFAULT_HEADER_PROFILE.accept_language.to_string());
    headers.insert("accept-encoding".to_string(), DEFAULT_HEADER_PROFILE.accept_encoding.to_string());
    
    // Beta header (test override available)
    let beta = env::var(TEST_BETA_HEADER).unwrap_or_else(|_| DEFAULT_HEADER_PROFILE.anthropic_beta.to_string());
    headers.insert("anthropic-beta".to_string(), beta);
    
    // Stream helper method header when stream=true
    if opts.stream {
        headers.insert("x-stainless-helper-method".to_string(), 
