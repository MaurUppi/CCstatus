...
mod tests {
    use super::*;
    use crate::core::network::proxy_health::client::{HealthCheckClient, HealthResponse};
    use std::time::Duration;

    #[derive(Default)]
    struct MockHealthClient {
        responses: std::collections::HashMap<String, Result<HealthResponse, String>>,
    }
    ...
    #[test]
    fn test_validate_redirect_host() {
        // Same host - should pass
        assert!(validate_redirect_host(
            "https://proxy.com/health",
            "https://proxy.com/api/health"
        ).is_ok());
