...
impl ErrorTracker {
    ...
    pub fn get_error_stats(&self, minutes: u32) -> ErrorStats {
        let cutoff_time = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or(Duration::ZERO)
            .as_millis() as u64 - (minutes as u64 * 60_000);

        let recent_errors: Vec<&ErrorEvent> = self.error_history
            .iter()
            .filter(|event| event.timestamp >= cutoff_time)
            .collect();

        let total_errors = recent_errors.len();
        let authentication_errors = recent_errors.iter()
            .filter(|e| matches!(e.error_type.as_str(), "authentication_error" | "permission_error"))
            .count();
        let rate_limit_errors = recent_errors.iter()
            .filter(|e| e.error_type == "rate_limit_error") 
            .count();
        let server_errors = recent_errors.iter()
            .filter(|e| matches!(e.error_type.as_str(), 
    ...
    pub fn cleanup_old_errors(&mut self, retention_hours: u32) {
        let cutoff_time = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or(Duration::ZERO)
            .as_millis() as u64 - (retention_hours as u64 * 60 * 60 * 1000);
    ...
    fn parse_jsonl_timestamp(timestamp_str: &str) -> u64 {
        ...
        if let Ok(parsed) = chrono::DateTime::<chrono::Utc>::from_str(&format!("{}Z", timestamp_str.trim_end_matches('Z'))) {
            return parsed.timestamp_millis() as u64;
        }
