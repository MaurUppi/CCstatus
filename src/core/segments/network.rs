...
impl NetworkSegmentWrapper {
    ...
    async fn run_orchestration(&self, input: &StatuslineInput) -> Result<String, NetworkError> {
        ...
        debug_logger.debug("NetworkWrapper", "Starting orchestration integration").await;
        ...
        if let Err(e) = segment.run(input.clone()).await {
            debug_logger.debug("NetworkWrapper", &format!(
                "Orchestration failed: {}", e
            )).await;
            return Err(e);
        }
        
        debug_logger.debug("NetworkWrapper", "Orchestration completed successfully").await;
